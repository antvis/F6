...
describe("graph", () => {
  ...
  it("find", () => {
    ...
    const findNode = globalGraph.find(
      "node",
      (node: any) => node.get("model").x === 100,
    );
  ...
  it("findAll", () => {
    ...
    const node3 = globalGraph.addItem("node", {
      id: "node3",
      x: 100,
      y: 100,
      size: 50,
...
describe("all node link center", () => {
  ...
  it("loop", () => {
    ...
    expect(edge4.getKeyShape().attr("path")[0][1]).toEqual(
      edge3.getKeyShape().attr("path")[1][5],
    );
    expect(edge4.getKeyShape().attr("path")[0][2]).toEqual(
      edge3.getKeyShape().attr("path")[1][6],
    );
...
describe("mapper fn", () => {
  ...
  it("node & edge mapper", () => {
    ...
    const node: Item = graph.addItem("node", {
      id: "node",
      x: 100,
      y: 100,
      value: 100,
    ...
    label = edge
      .getContainer()
      .find((element) => element.get("className") === "edge-label");
    expect(label).not.toBe(undefined);
  ...
  it("node & edge mapper with states", () => {
    ...
    expect(
      node
        .getContainer()
        .find((element) => element.get("className") === "node-label"),
    ).not.toBe(undefined);
