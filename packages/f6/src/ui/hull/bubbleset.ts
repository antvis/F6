...
const pickBestNeighbor = (item: Item, visited: Item[], nonMembers: Item[]): Item | null => {
  ...
  visited.forEach((neighbourItem) => {
    const itemP = { x: item.x, y: item.y };
    const neighbourItemP = { x: neighbourItem.x, y: neighbourItem.y };
...
function getRoute(
  ...
  const directLine = new Line(item.x, item.y, optimalNeighbor.x, optimalNeighbor.y);
...
export const genBubbleSet = (members: Item[], nonMembers: Item[], ops?: BubblesetCfg) => {
  ...
  const centroid = getPointsCenter(members.map((item) => ({ x: item.x, y: item.y })));
  // 按照到中心距离远近排序
  members = members.sort(
    (a, b) => squareDist({ x: a.x, y: a.y }, centroid) - squareDist({ x: b.x, y: b.y }, centroid),
  );
  const visited: Item[] = [];
  ...
  //   virtualEdges.push(new Line(e.getSource().x, e.getSource().y, e.getTarget().x, e.getTarget().y));
  // });
