...
QUnit.test('get/set prefixed util', function (assert) {
  assert.ok(
    _.isUndefined(Hammer.prefixed(window, 'FakeProperty')),
    'non existent property returns undefined',
  );

  window.webkitFakeProperty = 1337;
  assert.ok(
    Hammer.prefixed(window, 'FakeProperty') == 'webkitFakeProperty',
    'existent prefixed property returns the prefixed name',
  );
...
QUnit.test('Inherit objects', function (assert) {
  ...
  assert.ok(
    inst._super === Base.prototype,
    '_super is ref to prototype of Base',
  );
});

QUnit.test('toArray', function (assert) {
  assert.ok(
    _.isArray(Hammer.toArray({ 0: true, 1: 'second', length: 2 })),
    'converted an array-like object to an array',
  );
...
QUnit.test('inArray', function (assert) {
  assert.ok(
    Hammer.inArray([1, 2, 3, 4, 'hammer'], 'hammer') === 4,
    'found item and returned the index',
  );
  assert.ok(
    Hammer.inArray([1, 2, 3, 4, 'hammer'], 'notfound') === -1,
    'not found an item and returned -1',
  );
  assert.ok(
    Hammer.inArray([{ id: 2 }, { id: 24 }], '24', 'id') === 1,
    'find by key and return the index',
  );
  assert.ok(
    Hammer.inArray([{ id: 2 }, { id: 24 }], '22', 'id') === -1,
    'not found by key and return -1',
  );
});

QUnit.test('splitStr', function (assert) {
  assert.deepEqual(
    Hammer.splitStr(' a  b  c d   '),
    ['a', 'b', 'c', 'd'],
    'str split valid',
  );
});
...
QUnit.test('uniqueArray', function (assert) {
  assert.deepEqual(
    Hammer.uniqueArray([{ id: 1 }, { id: 2 }, { id: 2 }], 'id'),
    [{ id: 1 }, { id: 2 }],
