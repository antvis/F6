...
QUnit.test('recognize pan', function (assert) {
  ...
  Simulator.gestures.pan(
    el,
    { duration: 500, deltaX: 100, deltaY: 0 },
    function () {
      assert.deepEqual(
        events,
        {
...
QUnit.test('recognize swipe', function (assert) {
  ...
  Simulator.gestures.swipe(
    el,
    { duration: 300, deltaX: 400, deltaY: 0 },
    function () {
      assert.deepEqual(
        events,
        {
...
QUnit.test(
  "don't recognize pan and swipe when moving down, when only horizontal is allowed",
  function (assert) {
    var done = assert.async();
    assert.expect(1);
